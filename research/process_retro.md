...
- **Cycle 11 (value (earnings yield) on the point-in-time S&P500 universe):**
  - ✅ worked: the new sp500_pit_union runner path + chunked get_prices ran a ~700-name PIT factor cleanly (the cycle-10 harness suggestion, now paying off); drop rate fell to 11.5% (vs 40%); the placebo test DISCRIMINATED - killed gross profitability (noise) but passed value (real) - evidence it's a signal detector, not just a rejecter
  - ⚠️ friction / 🐞 bug: value's signal is real but sub-threshold on 2010+ large-cap PIT (low power); single-factor sleeves keep failing the significance bar even when the signal is real -> the framework is biased toward rejecting individually-weak-but-jointly-useful factors
  - 💡 suggestion (unapplied): add a MULTI-FACTOR COMPOSITE sleeve path (combine signal-specific-but-weak factors like value/quality/low-vol) so real-but-individually-sub-threshold signals can be evaluated together, not just standalone; this is likely where accessible-data alpha actually lives
- **Cycle 12 (multi-factor composite (value+quality+lowvol+momentum) on the PIT universe):**
  - ✅ worked: sp500_pit_union path ran a 4-factor PIT composite through the standard machinery cleanly; the composite-vs-parts test design was decisive; drop rate down to 4.2% (best yet); placebo correctly flagged the composite as not-signal-specific (p=0.11)
  - ⚠️ friction / 🐞 bug: three of four classic factors have NO signal on free large-cap post-2009 PIT data, so equal-weighting dilutes the one real factor (value) - the free-data equity-factor avenue is exhausted (cycles 9-12 all reject)
  - 💡 suggestion (unapplied): stop testing classic price/fundamental factors on free large-cap data (diminishing returns); pivot factor research to (a) the TEXT moat we built but never tested (edgar_text risk-factor/tone signals - less crowded), (b) paid breadth/small-cap+delisting data, or (c) consolidate the risk-managed portfolio as the product. Consider a lit-scout/orchestration note to deprioritize crowded large-cap factors.
